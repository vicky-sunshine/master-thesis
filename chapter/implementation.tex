...
POST   & /login\_data   & records user's log in/out activity   & \begin{tabular}[c]{@{}l@{}}action: String\\ userId: Number\\ userAccount: String\\ time: ISODate\end{tabular}                                                                                                                                                                                                              \\ \hline
POST   & /page\_data    & records user's trail on website      & \begin{tabular}[c]{@{}l@{}}action: String\\ userId: Number\\ userAccount: String\\ page: String\\ courseId: Number\\ chapterId: Number\\ time: ISODate\end{tabular}                                                                                                                                                        \\ \hline
POST   & /article\_data & records user's forum activity        & \begin{tabular}[c]{@{}l@{}}action: String\\ userName: String\\ userId: Number\\ userAccount: String\\ courseId: Number\\ object\_article\_info:\\ \{article\_Object\}\\ time: ISODate\end{tabular}                                                                                                                         \\ \hline
POST   & /post\_data    & records user's exercise activity     & \begin{tabular}[c]{@{}l@{}}exerciseId: Number\\ exerciseType: String\\ correctAns: String\\ execOrder: Number\\ courseId: Number\\ getScore: Boolean\\ execTotalCount: String\\ userId: Number\\ studentAns: Array\\ time: ISODate\end{tabular}                                                                            \\ \hline
POST   & /video\_data   & records user's video player activity & \begin{tabular}[c]{@{}l@{}}action: String\\ userId: Number\\ userAccount: String\\ videoStartTime: Number\\ videoEndTime: Number\\ videoId: Number\\ videoTotalTime: Number\\ courseId: Number\\ chapterId: Number\\ chapterVideoCount: Number\\ chapterVideoOrder: Number\\ playRate: Number\\ time: ISODate\end{tabular} \\ \hline
\end{tabular}}
\end{table}

\subsection{API's Record Usage of Videomark}
In order to understand whether the feature Videomark improve student's performance on learning, we recorded the usage data of Videomark by another set of APIs.
Table \ref{table:videomarkAPI} shows the detail of these APIs.

\begin{table}[H]
\centering
\caption{Videomark APIs}
\label{table:videomarkAPI}
\resizebox{\columnwidth}{!}{
\begin{tabular}{|l|l|l|l|}
\hline
method & url               & purpose                                                                                             & schema                                                                                                                                         \\ \hline
POST   & /cloud/saveLog    & \begin{tabular}[c]{@{}l@{}}record which keyword in keyword\\ cloud been clicked\end{tabular}        & \begin{tabular}[c]{@{}l@{}}userId: Number\\ courseId: Number\\ chapterId: Number\\ videoId: Number\\ word: String\\ time: ISODate\end{tabular} \\ \hline
POST   & /cloud/saveUrlLog & \begin{tabular}[c]{@{}l@{}}records which hyperlink in\\ hyperlink section been clicked\end{tabular} & \begin{tabular}[c]{@{}l@{}}userId: Number\\ courseId: Number\\ chapterId: Number\\ videoId: Number\\ time: ISODate\end{tabular}                \\ \hline
POST   & /cloud/openCloud  & \begin{tabular}[c]{@{}l@{}}records action that user open\\ keyword cloud\end{tabular}               & \begin{tabular}[c]{@{}l@{}}userId: Number\\ courseId: Number\\ chapterId: Number\\ videoId: Number\\ word: String\\ time: ISODate\end{tabular} \\ \hline
\end{tabular}}
\end{table}
...
Ajax, one of the feature of jQuery, make it possible to perform cross-browser Ajax requests using \code{\$.ajax}, Figure \ref{fig:ajaxpost} shows an code that ajax post a hash variable \code{data} to url \url{http://API_endpoint/saveData}.
If the request succeed \code{.done()} in line 8 will be triggered; otherwise \code{.fail()} in line 10 will be triggered.
